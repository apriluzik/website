...
<!--                memo테이블에 저장하는 insert.php로 제출할 form요소 필요-->
...
//            내림차순(num넘버가 큰것부터 정렬)
            ...
            $rowNum= mysqli_num_rows($result);//전체글 수

            //한 화면에 표시될 글의 수 지정
            $scale = 5;

            //한페이지에 5개씩 표시되므로 전체 글수를 기반으로 페이지 수를 계산
            //(1~5 : 1page, 6~10: 2page..)

//            if( $rowNum%$scale==0){$pageNum= $rowNum/$scale;
//            }else{$pageNum= floor($rowNum/$scale)+1;}

            $pageNum = floor($rowNum/$scale+0.99);
            if($pageNum==0) $pageNum=1; // 글이 없다면 ..최소 1페이지

            //현재 페이지번호
            // 이문서 가장 아레에 페이지 번호 표시 영역
            // 이때 페이지를 선택하면 get방ㅅ기으로 그 페이지의
            // 글 목록을 보여줌.
            ...
            //$i< $start+$scale && $i<$rowNum ==> 글이 13개일수도 있으니 ..$rowNum을 넘지않게
            for($i = $start ; $i< $start+$scale && $i<$rowNum; $i++){
                ...
                //content의 내용은 textarea에 입력햇음..
                //줄바꿈의 문자가 \n으로 되어있음. <br/>로 바꾸기.
                $memo_content= nl2br($memo_content);

                // 한줄 읽어올 때 마다 출력..반복문 돌때마다..
                // echo >>>>>
                ?>
<!--                HTML -->
